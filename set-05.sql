-- ProblemSet<Problem set: 5>, November <28/11/2017> 
...
select m.m_name as organiser_name,m.m_no,c.concert_no from musician m 
join concert c on m.m_no=c.concert_organiser 
...
select m.m_name,place_country from place p 
 join musician m on m.born_in=p.place_no 
join concert c on m.m_no=c.concert_organiser 
...
select instrument,avg(count) as avg_musicians from 
(select instrument,count( perf_is) as count from performer 
group by instrument) 
group by instrument
...
select b.band_name,p.band_id,count(p.player) as no_of_players from plays_in p 
join band b on p.band_id=b.band_no 
group by p.band_id having no_of_players>
(select (sum(count)/count(band_id)) 
from (select count(band_id) as count,band_id
 from plays_in group by band_id ));
...
select m.m_name,p.place_town from musician m 
join place p on m.born_in=p.place_no
 where place_town=(select place_town from place p join 
...
select distinct(b.band_name),c.concert_organiser as conductor,b.band_contact from band b
 join concert c on c.concert_organiser=b.band_contact 
where b.band_contact in 
(select concert_organiser from concert where concert_venue like'%royal albert hall%'  

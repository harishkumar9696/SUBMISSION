-- ProblemSetBonus, November <28> <2017> 
-- Submission by <h.f.chandrasekaran>  
...
select distinct p.fname from Person p, Movie m, Movie m1, Cast c, Cast c1 
where c.pid = p.id and c.mid = m.id and m.year 
between 1850 and 1900 and c1.pid = p.id
and c1.mid = m1.id 
and m1.year between 1901 and 1950;

/*2.List all the directors who directed a film in a leap year*/

select fname,m.year from  Director d join Moviedirector md on d.id=md.did 
join Movie m on m.id=md.mid
 where m.year % 4=0 and 
m.year % 100 !=0 or 
m.year % 400=0;
...
select name from Movie where year in 
(select year from Movie
 where name='Shrek (2001)') and rank>5;

/*4.List first name and last name of all the actors who played in the movie 'Officer 444 (1926)'*/

select fname,lname from Person p join Cast c on c.pid=p.id 
join Movie m on mid=c.mid 
where m.name='Officer 444 (1926)'; 

/*5.List all directors in descending order of the number of films they directed*/

select fname,lname,count(md.mid) as no_of_flims from Director d 
join Moviedirector md on md.did=d.id 
group by d.id 
order by no_of_flims DESC;
...
select p.fname ,p.lname from Person p join Cast c on c.pid=p.id 
join Movie m on c.mid=m.id 
where m.year < 1960;

/*11.Find the films with more women actors than men.*/

select * from(select m.name  from Movie m join Cast c on c.mid=m.id  join Person p on c.pid=p.id 
and p.gender='F' 
group by m.id having count(*)>
(select count(*) from Movie m ,Cast c1,Person p 
where c1.mid = m.id AND c1.pid = p.id 
AND p.gender='M'));
...
select * from(select gender,p.id,m.id from Person p ,Cast c,Movie m
 where  c.mid=m.id and c.pid=p.id 
group by m.id having m.id=p.id) ;
